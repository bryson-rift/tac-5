...
def generate_test_query_with_openai(schema_info: Dict[str, Any]) -> str:
    """
    Generate a test query using OpenAI API based on database schema
    """
    try:
        api_key = os.environ.get("OPENAI_API_KEY")
        if not api_key:
...
Generate an interesting natural language query that would be useful to run against this database. The query should showcase the system's ability to convert natural language to SQL.

Requirements:
- Maximum 2 sentences in natural language
- Only suggest queries that retrieve data (SELECT operations, no INSERT/UPDATE/DELETE/DROP)
- Make the query interesting and showcase database capabilities
- The query should produce meaningful results

Example: "Show me all users who signed up in the last month and their email addresses."

Your query (natural language only, no SQL):"""
        ...
        raise Exception(f"Error generating test query with OpenAI: {str(e)}")

def generate_test_query_with_anthropic(schema_info: Dict[str, Any]) -> str:
    """
    Generate a test query using Anthropic API based on database schema
    """
    try:
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
...
Your query (natural language only, no SQL):"""
        ...
        raise Exception(f"Error generating test query with Anthropic: {str(e)}")

def generate_test_query(schema_info: Dict[str, Any]) -> str:
    """
    Generate a test query using available LLM provider.
    ...
    if openai_key:
        return generate_test_query_with_openai(schema_info)
    elif anthropic_key:
        return generate_test_query_with_anthropic(schema_info)
    else:
        raise Exception("No LLM API key available. Set OPENAI_API_KEY or ANTHROPIC_API_KEY environment variable.")